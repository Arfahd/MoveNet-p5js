...
async function setup() {
  ...
  try {
    ...
    video.elt.addEventListener('loadedmetadata', () => {
      ...
      // Auto-start video playback for testing
      video.loop();
    });
...
function setupEventListeners() {
  ...
  // Video source selector (vidinput branch only)
  const videoSourceSelect = document.getElementById('videoSource');
  if (videoSourceSelect) {
    videoSourceSelect.addEventListener('change', (e) => {
      const newSource = e.target.value;
      console.log(`Switching to video: ${newSource}`);
      ...
      video = createVideo(newSource, () => {
        console.log("New video file loaded successfully");
      });
      video.volume(0);
      video.hide();
      ...
      video.elt.addEventListener('loadedmetadata', () => {
        const videoWidth = video.elt.videoWidth;
        const videoHeight = video.elt.videoHeight;
        console.log(`New video resolution: ${videoWidth}x${videoHeight}`);
